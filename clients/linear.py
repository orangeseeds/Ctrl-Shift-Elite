...
# Loading dataset
df = pd.read_csv("./clients/train.csv")
y_train = df['cardio']
x_train = df.drop(columns=['cardio', 'id'], axis=1)

df_test = pd.read_csv("./clients/test.csv")
...
class FlowerClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        filtered_hist = {key: value for key, value in hist.items() if key in [
            "loss", "accuracy"]}

        # Display "Client-Side Training History:" in bold and larger size
        st.markdown("<h3><b>Client-Side Training History:</b></h3>",
    ...
    def evaluate(self, parameters, config):
        ...
        st.markdown("<h3><b>Global Server Accuracy:</b></h3>",
                    unsafe_allow_html=True)
        # Display global server accuracy using Streamlit
        st.write("Global accuracy:", accuracy)
